...
namespace Alkl.Thira.Constraints
{
    public class DefaultMovementConstraints : IMovementConstraints
    {
        ...
        public void CheckArguments(Player player, Field fieldFrom, Field fieldTo)
        {
            if (player == null)
            {
                throw new PlayerDoesNotExistException(null, fieldFrom, fieldTo);
            }

            if (fieldFrom == null)
            {
                throw new SourceFieldDoesNotExistException(player, null, fieldTo);
            }

            if (fieldTo == null)
            {
                throw new DestinationFieldDoesNotExistException(player, fieldFrom);
            }

            if (fieldFrom.Builder == null)
            {
